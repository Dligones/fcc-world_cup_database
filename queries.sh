...
echo  "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games gw ON teams.team_id = gw.winner_id FULL JOIN games go ON teams.team_id = go.opponent_id WHERE (gw.round = 'Eighth-Final' AND gw.year = 2014) OR (go.round = 'Eighth-Final' AND go.year = 2014)")"
